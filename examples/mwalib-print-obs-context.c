...
int main(int argc, char *argv[])
{
    ...
    mwalibMetafitsMetadata *metadata = NULL;
    if (mwalib_metafits_metadata_get(NULL, correlator_context, &metadata, error_message, ERROR_MESSAGE_LEN) == EXIT_SUCCESS)
    {
    ...
    mwalibTimeStep *ts0 = NULL;  // Should return first timestep
    if (mwalib_correlator_timestep_get(correlator_context, 0, &ts0, error_message, ERROR_MESSAGE_LEN) == EXIT_SUCCESS)
    {
    ...
    mwalibAntenna *ant0 = NULL;
    if (mwalib_antenna_get(metafits_context, 0, &ant0, error_message, ERROR_MESSAGE_LEN) == EXIT_SUCCESS)
    {
    ...
    if (mwalib_rfinput_get(metafits_context, 300, &rf0, error_message, ERROR_MESSAGE_LEN) == EXIT_SUCCESS)
    {
